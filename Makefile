...
pkgdatadir = $(datadir)/frontline
pkglibdir = $(libdir)/frontline
pkgincludedir = $(includedir)/frontline
...
PRE_UNINSTALL = :
POST_UNINSTALL = :
AUTOTRACE_CFLAGS = -I/usr/local/include
AUTOTRACE_CONFIG = /usr/local/bin/autotrace-config
AUTOTRACE_LIBS = -L/usr/local/lib -L/usr/lib -L/usr/X11R6/lib -lautotrace -lMagick -ltiff -lfreetype -ljpeg -lpng -ldpstk -ldps -lXext -lXt -lSM -lICE -lX11 -lbz2 -lz -lpthread -lming -lm
CC = gcc
CPP = gcc -E
...
CONFIG_CLEAN_FILES =  frontline-config frontline.pc
SCRIPTS =  $(bin_SCRIPTS)

DATA =  $(Graphics_DATA) $(Mime_DATA) $(appicon_DATA) $(m4data_DATA) \
$(pkgconfig_DATA)

DIST_COMMON =  README AUTHORS COPYING ChangeLog INSTALL Makefile.am \
Makefile.in NEWS TODO aclocal.m4 configure configure.in \
frontline-config.in frontline.pc.in install-sh missing mkinstalldirs
...
$(srcdir)/configure: $(srcdir)/configure.in $(ACLOCAL_M4) $(CONFIGURE_DEPENDENCIES)
	cd $(srcdir) && $(AUTOCONF)
frontline-config: $(top_builddir)/config.status frontline-config.in
	cd $(top_builddir) && CONFIG_FILES=$@ CONFIG_HEADERS= $(SHELL) ./config.status
frontline.pc: $(top_builddir)/config.status frontline.pc.in
	cd $(top_builddir) && CONFIG_FILES=$@ CONFIG_HEADERS= $(SHELL) ./config.status
...
install-data-am: install-GraphicsDATA install-MimeDATA \
		install-appiconDATA install-m4dataDATA \
...
uninstall-am: uninstall-binSCRIPTS uninstall-GraphicsDATA \
		uninstall-MimeDATA uninstall-appiconDATA \
...
installdirs-am:
	$(mkinstalldirs)  $(DESTDIR)$(bindir) $(DESTDIR)$(Graphicsdir) \
		$(DESTDIR)$(Mimedir) $(DESTDIR)$(appicondir) \
...
.PHONY: uninstall-binSCRIPTS install-binSCRIPTS uninstall-GraphicsDATA \
install-GraphicsDATA uninstall-MimeDATA install-MimeDATA \
uninstall-appiconDATA install-appiconDATA uninstall-m4dataDATA \
